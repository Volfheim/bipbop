// package core provides a multiplexer for multiple streams over a single connection.
package core
...
func (m *Multiplexer) SendData(sid uint16, data []byte) error { //nolint:revive
	...
	if totalChunks > 10 {
		log.Printf("SendData: sid=%d, size=%d bytes, chunks=%d", sid, len(data), totalChunks)
	}
...
func (m *Multiplexer) getOrCreateStream(sid uint16, clientID uint32) *Stream {
	...
	if !exists {
		...
		m.streams[sid] = stream
		select { case m.acceptCh <- sid: default: }
		return stream
	}
...
func (m *Multiplexer) applyOutOfOrder(stream *Stream, sid uint16, clientID uint32) {
	for {
		...
		stream.nextSeq++
		log.Printf("Applied out-of-order packet sid=%d seq=%d", sid, stream.nextSeq-1)
	}
}
...
func (m *Multiplexer) handleControlFrame(control ControlFrame) {
	...
	default:
		log.Printf("Unknown mux control frame type=%d clientID=%d", control.Type, control.ClientID)
	}
}
...
func (m *Multiplexer) Reset() { //nolint:revive
	...
	m.acceptCh = make(chan uint16, 5000)
	m.sendSeqMu.Unlock()
}
...
func (c *MuxConn) Close() error {
	...
	c.mux.CleanupDataChannel(c.sid)
	return nil
}
